...
def setIC(ICvector = np.array([10e-12, 1.45e-6, 2e-8, 1e-8, 1e-8/100, 7e-10, 9e-8, 1.6e-7, 3e-8, 3.45e-6, 2.5e-9])):
    """
    Converts a list of factor levels into an initial condition vector for the Danforth model assuming 0 concnetration for all remaining species. \n
    Inputs: \n
    ...
    Outputs: \n
    IC - Vector of initial conditions for the Danforth model
    """
    IC = np.zeros(34);
...
def getRates():
    """
    Get the reaction rates for simulating the Danforth model. \n
    Inputs: \n
    None \n
    Outputs: \n
    List of reaction rates
    """
    k = np.array([3.1e-3, 3.2e6, 3.1e-3, 2.3e7, 4.4e5, 1.3e7, 2.3e4, 1.05, 2.5e7, 6, 19, 2.2e7, 2.4, 1e7, 1.8, 7.5e3, 2.0e7, 5e-3, 1e7, 1e-3, 1e8, 8.2, 2.2e4, 6e-3, 1e-3, 2e7, 0.2, 4e8, 103, 1e8, 63.5, 2.3e8, 3.6e-4, 9e5, 1.1e-4, 3.2e8, 5e7, 4.2e3, 7.1e3, 4.9e2, 7.1e3, 2.3e2, 5.7e3, 3e6]);
    return k

def getThr(k,y,maxt):
    """
    Simulate the Danforth model and return the thrombin generation curve. \n
    Inputs: \n
...
def plotThr(k,y,maxt):
    """
    Simulate the Danforth model and plots the thrombin generation curve. \n
    Inputs: \n
    k - List of reaction rates, \n
    y - Initial condition vector from setIC, \n
    maxt - Simulation time specified in seconds (1200s = 20min) \n
    ...
    plt.title("Danforth Model")
    plt.show()

def test():
    """
    Simulate the Danforth model and plots the thrombin generation curve for default initial concentrations (Requires no inputs). \n
    Inputs: \n
    None \n
    Outputs: \n
    None, plot of thrombin generation curve is displayed.
...
def ODESolver(k,c0,maxt):
    """
    Simulate the Danforth model and returns the ODE solution object. \n
    Inputs: \n
    k - List of reaction rates, \n
    y - Initial condition vector from setIC, \n
    maxt - Simulation time specified in seconds (1200s = 20min) \n
...
def ODE(t,y,k):
    """
    Evaluate the ODE of the Danforth model. \n
    Inputs: \n
